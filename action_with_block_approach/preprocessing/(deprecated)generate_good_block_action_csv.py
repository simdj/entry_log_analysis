...
if __name__ == "__main__":
    raw_block_action_csv_path='../data/raw_block_action.csv'
    raw_block_action_csv_path_filtered='../data/good_block_action.csv'

    # step 1) building user's metadata
    building_filter(raw_block_action_csv_path)

    # step 2) get ready to read 
    csv_to_read = open(raw_block_action_csv_path,'rb');
    reader = csv.reader(csv_to_read);
    reader.next() #skip the first line

    # step 3) get ready to write
    csv_to_write = open(raw_block_action_csv_path_filtered, 'w');
