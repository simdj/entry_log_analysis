// usage
// mongo --quiet entry_2016Summer generate_raw_data.js > raw_data.csv
...
// header_row=['key','id','lecture','run', '+normal', '+repeat','+if','-normal', '-repeat','-if','err_incomplete', 'err_without_ct', 'err_etc', '5#', '10#', '30#', '60#','300#','long#']
// header_row = meta_header.concat(action_header).concat(err_header).concat(time_header)
...
// input action : only run, insert, separate, !!fail!!
// output : one item of set which is {run, +normal, -normal,+if, -if, +repeat, -repeat, err_incomplete, err_without_ct, err_etc}
build_simple_action_log_obj = function(action){
	...
	switch(action_name){
		...
		case COMMAND_TYPES.fail:
			fail_type = action.data[0].value*1 //18,20,...
			
			// 'fail~'+action_timestamp
			action_format='err_etc~'+action_timestamp
			if((ERROR_TYPES.INCOMPLETE).indexOf(fail_type)>-1){
				action_format='err_incomplete~'+action_timestamp
			}else if((ERROR_TYPES.WITHOUT_CT).indexOf(fail_type)>-1){
				action_format='err_without_ct~'+action_timestamp
			}	
			
			return {'action':action_format, 'created':new Date(action_timestamp) }
		default:
			return false
	}
...
record_user_lecture_event = function(obj){
	...
	if(!csv_table[user+','+stage]){
		//new!!
		csv_table[user+','+stage]={ user:user, lecture:stage, event_list:[], start:false, success:false, success_time:new Date()}
	}

	// recording part!!!
	if(obj.type=='startStage' ){
		csv_table[user+','+stage].start=true
		return false
...
print_compressed_csv_row_data = function(user, lecture, action_list){
	...
	// var ret_err = Array(err_header.length).fill(0); 
	// var ret_time = Array(time_header.length).fill(0); 
	
	// // 1. action and interval count fill
	// base_time = ''
	...
	// for(i in action_list){
	// 	col_name = action_list[i].split('~')[0]
	// 	col_index = err_header.indexOf(col_name)
	// 	if(col_index<0){
	// 		continue
	// 	}
	// 	ret_err[col_index]++
	...
	// print raw csv data
	// print(ret_meta.concat(ret_action).concat(ret_err).concat(ret_time).concat(action_list))
	print(ret_meta.concat(action_list))
}

print_csv_table = function(){
	for(index in csv_table){
		var row = csv_table[index]
		if(row.success && row.start){
...
// 	{
//     	$and : [
//     		{'actions':{$elemMatch:{name:'fail'}}}
//    			,{'actions':{$elemMatch:{data:{$elemMatch:{value:13}}}}}
//     	]
//     }
//     ,{
//     	"actions.data.$":1,"actions.data.value":1
//     }
// )
